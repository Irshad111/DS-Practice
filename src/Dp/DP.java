...
public class DP {
	...
	public static void main(String[] args) {
		...
		String str = "helppreanadkada";
		System.out.println(palindromicpartitionTD(str, 0, str.length() - 1, new int[str.length()][str.length()]));
...
// 3.
  ...
  // 8. 
	...
	public static int noWayToCoinChange(int arr[], int n) {
		int size = arr.length;
		int strg[][] = new int[size + 1][n + 1];
		// if n=0 then there is one solution do not include any coin
		...
		for (int row = 1; row <= size; row++) {
			for (int col = 1; col <= n; col++) {
				int include = 0;
				if (col >= arr[row - 1]) {
					include = strg[row][col - arr[row - 1]];
				}
				int exclude = strg[row - 1][col];
  ...
  // 13.
	...
	public static int palindromicpartitionTD(String str, int si, int ei, int[][] strg) {
		...
		for (int k = si; k <= ei - 1; k++) {
			int fc = palindromicpartitionTD(str, si, k, strg);

			int sc = palindromicpartitionTD(str, k + 1, ei, strg);
			int res = fc + sc + 1;
			if (res < min) {
				min = res;
			}
	...
	public static int mincoinBU(int[] coin, int amount) {
		...
		for (int rupay = 1; rupay <= amount; rupay++) {
			strg[rupay] = Integer.MAX_VALUE;
			for (int i = 0; i < coin.length; i++) {
				if (rupay >= coin[i]) {
					int ans = strg[rupay - coin[i]];
					if (ans != Integer.MAX_VALUE && ans + 1 < strg[rupay]) {
						strg[rupay] = ans + 1;
					}
...
//  
	...
	// usinng dp time=o(n2) and space o(n) if we can not jump then return -1
	...
	public static String justify(String[] word, int width) {
		...
		for (int i = 0; i < word.length; i++) {
			...
			for (int j = i + 1; j < word.length; j++) {
				cost[i][j] = cost[i][j - 1] - word[j].length() - 1;// -1 for space
			}
		}
		// for squere of cost
		for (int i = 0; i < word.length; i++) {
	...
	int wildCard(String pattern, String str) {
		...
		boolean strg[][] = new boolean[s.length + 1][idx + 1];
		strg[0][0] = true;
		if (idx > 0 && p[0] == '*') {
			strg[0][1] = true;
		}
    ...
    boolean [][]strg=new boolean[A.length()+1][B.length()+1];
    for(int i=0;i<=A.length();i++){
        for(int j=0;j<=B.length();j++){
            ...
            if(C.charAt(i+j-1)==A.charAt(i-1) && strg[i-1][j]==true)
                strg[i][j]=true;
            
            if(C.charAt(i+j-1)==B.charAt(j-1) && strg[i][j-1]==true)
                strg[i][j]=true;
            }
        }
    }
    return strg[A.length()][B.length()];
	}
	...
	public int burstingballoonsBUDP(int[] nums) {
	...
	public long catlan(int n) {
		long c=BinomialCoefficient(2*n,n);
		return c/n+1;
	}
	public long BinomialCoefficient(int n,int r) {
		if(r>n-r)
			r=n-r;
		long res=1;
		for(int i=0;i<r;i++) {
