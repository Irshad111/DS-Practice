...
{  
    ...
    double res=0;
    if(exp.charAt(i)=='-'){
        res=-1*value(exp.charAt(i+1));
        i+=2;
    }else{
        res=value(exp.charAt(i));
        i+=1;
    }
  
    // Traverse the remaining characters in pairs  
    for (; i<exp.length(); i += 2)  
    {  
        ...
        double temp=value(opd);
        int j=i+2;
        while(j<exp.length() && (exp.charAt(j)=='*' || exp.charAt(j)=='/')){
            char nextopr=exp.charAt(j);
           char  nextopd = exp.charAt(j+1); 
           if (isOperand(nextopd) == false) return -1; 
            
           if (nextopr == '*') temp *= value(nextopd);  
           else if (nextopr == '/') temp /= value(nextopd); 
